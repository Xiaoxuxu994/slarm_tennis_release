...
def test_default_is_four_gpu_pixel_finetune_without_resume(launch):
    lines = launch()
    assert "EXEC=torchrun" in lines
    assert f"CWD={ROOT}" in lines
    assert "GPUS=0,1,2,3" in lines
    assert "ARG=--nproc_per_node=4" in lines
    assert f"ARG=--config={DEFAULT_CONFIG}" in lines
    assert "ARG=--enable_tensorboard" in lines
    assert "ARG=--auto_resume" not in lines
    assert "ARG=--resume_from" not in lines


def test_environment_can_select_other_four_gpus_and_another_config(launch):
    lines = launch({"GPUS": "4,5,6,7", "CONFIG": OTHER_CONFIG})
    assert "GPUS=4,5,6,7" in lines
    assert "ARG=--nproc_per_node=4" in lines
    assert f"ARG=--config={OTHER_CONFIG}" in lines
...
def test_the_default_config_exists():
    """A launcher whose default names a deleted config only fails at run time."""
    assert (ROOT / DEFAULT_CONFIG).is_file()
    assert (ROOT / OTHER_CONFIG).is_file()
